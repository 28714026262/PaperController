 ...
 * @LastEditTime: 2022-10-25 10:44:45
...
问题：需要一个基准测试数据集，该数据集提供大量的标签和代码对，用于评估建议的解决方案。
...
-无标准基准数据集  
**58**提到9个用C编程语言编写的开源软件项目、提供了功能级别和文件级别的标签
...
树结构LSTM（TS-LSTM）、递归神经网络、图神经网络（如图卷积网络（GCN）[48]和图自动编码器（GAE））
**代码存储库的提交消息[79]、对一段代码进行描述/解释的注释以及代码评审注释**github可以使用的信息

-语义鸿沟  
特征工程+高质量的特征较少依赖于复杂的表达模型（需大量训练）-》定义反映特定类型漏洞特征的特征集
-》针对性的56、121√  
嵌入技术-》表示\描述漏洞的特征：  
基于频率的（例如，n-gram模型（有限词））和基于预测的（例如Word2vec模型[67]，[68]（无法捕获目标单词与其上下文之间相互依存的意义））单词嵌入解决方案

-人类理解
LIME10[85]被提议通过在预测样本周围局部学习简单且可解释的模型（例如，线性或决策树模型）来为任何模型提供可解释性（特征提取器时不可用）
