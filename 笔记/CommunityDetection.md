 ...
 * @LastEditTime: 2023-12-20 10:58:07
...
- [社群探测 CommunityDetection](#社群探测-communitydetection)
  - [目录](#目录)
  - [常见社群探测分类](#常见社群探测分类)
  - [基于学习的社区检测](#基于学习的社区检测)
    - [概率图形模型](#概率图形模型)
      - [有向图](#有向图)
        - [随机block model（SBM）](#随机block-modelsbm)
        - [主题模型](#主题模型)
        - [矩阵分解](#矩阵分解)
      - [无向图](#无向图)
      - [混合图](#混合图)
...
将隐藏表示视为具有自身先验分布的潜在变量。在变分推理中，它利用了潜变量的真后验p。

- ARVGA的主题不仅是最小化网络结构的重构误差，而且是强制潜在代码与先验分布相匹配；
- 在VGAECD[111]的训练过程中，重建损失偏离了其聚类的主要目标。新的VGAECD[113]通过引入对偶变分目标来纠正这一问题；
- 一种基于伽玛梯VAE的深度生成模型，该模型通过多层随机潜在变量推断节点的多层嵌入，以提高社区检测的性能。
- NetVAE，该方法使用一个编码器和一个双解码器，以及两种不同的生成机制来分别重构网络拓扑和节点属性。

#### 基于生成对抗网络

通过生成器和鉴别器之间的对抗性游戏来检测社区。

- 一种称为CommunityGAN的新方法，采用隶属图模型（AGM）的思想，通过在网络基序级生成器和鉴别器之间引入极大极小竞争来提高性能。它首先通过为每个节点-社区对分配一个非负因子来组成节点的一些表示向量，该非负因子表示节点对社区的隶属度，然后通过专门设计的GAN来优化这种表示，以检测社区。
- 一种利用生成对抗性学习（SEAL）进行种子扩展的新方法。它使用鉴别器来预测社区是否真实，并使用生成器来构建社区，通过隐式拟合真实社区的特征来欺骗鉴别器，以学习社区检测的启发式方法。
- 一种对抗性图自动编码器（AGAE）方法，该方法将集成聚类纳入深度图嵌入过程，以利用对抗性正则化子指导网络训练。
- 使用聚类算法（如K-means）来导出嵌入，以获取结果社区。
- 利用表示机制上的对抗性思想来获取下游任务的节点表示。
- 一个联合对抗性网络嵌入（JANE）模型，该模型可以联合区分嵌入、拓扑信息和节点的真实和虚假组合属性，以提高节点嵌入和网络分析的性能。

#### 基于GCN

过特征在网络拓扑上的传播和聚合来提取社区。

- 引入了一个名为JGECD的无监督模型，通过联合GCN嵌入进行社区检测。它由三个模块组成，一个双编码器，使用原始属性网络及其变体导出两个嵌入；社团检测模块，其堆叠在双编码器的顶部以检测社团；以及拓扑重构模块，用于重构网络拓扑。
- 一种新的GCN方法来扩展JGE-CD，该方法将MRFasGCN（稍后将在第4.4节中讨论）作为编码器，并利用以社区为中心的双编码器分别重建网络拓扑和节点属性，以执行无监督的社区检测。
- 一种异构的时态GCN，即HTGCN，用于从异构和时态网络中检测群落。首先通过采用异构GCN来获得每个时间步长的每个异构网络的特征表示，然后利用残差压缩聚合机制来表达社区的静态和动态特征。
- MRFasGCN
- GMNN
...
- MRFasGCN，它将GCN与MRF相结合，以解决属性网络中的半监督社区检测问题。该方法首先通过添加一元势和属性信息将NetMRF扩展为扩展MRF（eMRF），然后重新参数化MRF模型，使其适合GCN架构。
- 图马尔可夫神经网络（GMNN），它结合了统计关系学习和图神经网络的优点。GMNN能够学习有效的节点表示和不同节点之间的标签依赖性模型，从而完成半监督节点分类的任务。
- 模型参数可以通过使用伪似然变分期望最大化[133]来优化对数似然函数的证据下界（ELBO）来学习

## 常见的图规模

![图 5](../images2/a95cc0e0d3ed80d4c8ecd06605fea9e5c2727e5a5b5df62caf92e2e90414edd6.png)  
